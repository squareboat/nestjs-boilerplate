...
@Injectable()
@Command('gen:lib {name}', {
  desc: 'Command to generate boilerplate friendly library',
})
export class GenLib {
  public async handle(_cli: ConsoleIO): Promise<void> {
    ...
    const nestCliJson: {
      ...
      monorepo: Boolean;
    } = JSON.parse(nestCliContent);
    if (nestCliJson.monorepo) await Create.lib(appName, _cli);
    else {
      _cli.error('This command can only be run in monorepo mode');
      _cli.info(
        `Hint: Run "node cli convert:monoRepo ${appName}" to convert to monorepo mode`,
      );
    }
  }
}
